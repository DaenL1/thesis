...
export default function MemberDashboard() {
  ...
  useEffect(() => {
    const fetchMemberData = async () => {
      if (user !== undefined) { // Check if user from useAuth is loaded (not undefined)
        try {
          setIsLoading(true);
          if (user === null) { // User is explicitly not logged in
            setError("You must be logged in to view this page.");
            // Redirect to login if not authenticated
            // router.push('/login'); // Uncomment if you want automatic redirect
          } else {
