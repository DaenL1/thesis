...
export default function AdminInventoryPage() {
  ...
  const getStatusPriority = (product: Product): number => {
    if (product.expiryDate && isExpired(product.expiryDate)) {
      return 0; // Expired products first
    }
    if (product.expiryDate && isExpiringSoon(product.expiryDate)) {
      return 1; // Expiring soon products next
    }
    if (product.isActive === false) {
      return 2; // Archived products after expiry issues
    }
    return 3; // Active products last (default)
  };
